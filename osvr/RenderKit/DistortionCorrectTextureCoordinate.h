...
namespace renderkit {
    ...
    ///
    /// @return New coordinates on success, unchanged coordinates on failure.
    inline Float2 DistortionCorrectTextureCoordinate(
        ...
        case DistortionParameters::mono_point_samples: {
            ...
            ret = interpolators[0]->interpolateNearestPoints(xN, yN);
        } break;
        case DistortionParameters::rgb_point_samples: {
            ...
            // product magnitude far enough from 1).  If we don't find such
            // points, we just go with the values from the closest point.
            ret = interpolators[color]->interpolateNearestPoints(xN, yN);
        } break;
        default:
            break;
        }
